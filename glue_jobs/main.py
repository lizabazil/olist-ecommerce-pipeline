...
print(f"customers df, deleted duplicates={customers_before_deleting_duplicates - customers_after_deleting_duplicates}")
...
print(f"customers df, deleted duplicates={df_before - df_after}")
...
order_items_df = order_items_df.withColumn("shipping_limit_date", f.when(f.col("shipping_limit_date").isin("0", None), None)
                                           .otherwise(f.to_date(f.col("shipping_limit_date"), "yyyy-MM-dd HH:mm:ss")))
...
order_reviews_df = order_reviews_df.withColumn("review_creation_date", f.when(f.col("review_creation_date").isin("0", None), None)
                                           .otherwise(f.to_date(f.col("review_creation_date"), "yyyy-MM-dd HH:mm:ss")))
...
order_reviews_df = order_reviews_df.withColumn("review_answer_timestamp", f.when(f.col("review_answer_timestamp").isin("0", None), None)
                                           .otherwise(f.to_date(f.col("review_answer_timestamp"), "yyyy-MM-dd HH:mm:ss")))
